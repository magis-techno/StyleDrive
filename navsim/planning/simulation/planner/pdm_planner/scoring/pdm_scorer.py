...
class PDMScorer:
    ...
    def _aggregate_scores(self, style) -> npt.NDArray[np.float64]:
        ...
        if max_raw_progress > self._config.progress_distance_threshold:
            ...
            deviation = (raw_progress - baseline) / reference
            squared_error = deviation ** 2
            normalized_progress = np.maximum(1 - alpha * squared_error, 0)
        else:
    ...
    def _calculate_ttc(self, style):
        ...
        def style_aware_ttc_score(ttc_scores: np.ndarray, style: str) -> np.ndarray:
            ...
            scores = (ttc_scores >= threshold).astype(float)
            return scores

        ttc_scores = np.ones(self._num_proposals, dtype=np.float64) * (
            self.proposal_sampling.interval_length * 15
        )
            ...
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                ...
                for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                    ...
                    if is_agent_ahead(ego_rear_axle, track_state) or (
                        ...
                        collision_time = float(future_time_idx) * self.proposal_sampling.interval_length
    ...
    def _calculate_is_comfortable(self, style) -> None:
        ...
        if style == 'A':
            style_comfort_thresholds = ComfortThresholds(
                max_abs_mag_jerk=8.4,
                max_abs_lat_accel=4.8,
                max_lon_accel=2.4,
                min_lon_accel=-3.84,
                max_abs_yaw_accel=2.16,
                max_abs_lon_jerk=4.32,
                max_abs_yaw_rate=0.84
            )
        elif style == 'C':
            style_comfort_thresholds = ComfortThresholds(
                max_abs_mag_jerk=5.6,
                max_abs_lat_accel=3.2,
                max_lon_accel=1.6,
                min_lon_accel=-2.56,
                max_abs_yaw_accel=1.44,
                max_abs_lon_jerk=2.88,
                max_abs_yaw_rate=0.56
            )
        else:
            style_comfort_thresholds = ComfortThresholds()
